...
// The scheduler class started simple, but at some point having it all in the header is too ridiculous. Migrate non
// perf-intensive calls here! (all but sync, really)
...
bool IsSleepingInFutex(uint32_t linuxPid, uint32_t linuxTid, uintptr_t futexAddr) {
    ...
    bool match = argList.size() >= 2 && strtoul(argList[0].c_str(), nullptr, 0) == SYS_futex &&
                 (uintptr_t)strtoul(argList[1].c_str(), nullptr, 0) == futexAddr;
    // info("%s | %s | SYS_futex = %d futexAddr = 0x%lx | match = %d ", ss.str().c_str(), Str(argList).c_str(),
    // SYS_futex, futexAddr, match);
    return match;
}
...
void Scheduler::watchdogThreadFunc() {
    ...
    while (true) {
        ...
        // if (lastPhase == curPhase && scheduledThreads == outQueue.size() && !sleepQueue.empty()) info("Mult %d
        // curPhase %ld", multiplier, curPhase);
        ...
        if (lastPhase == curPhase && !fakeLeaves.empty() && (fakeLeaves.front()->th->futexJoin.action != FJA_WAKE)) {
            if (++fakeLeaveStalls >= WATCHDOG_STALL_THRESHOLD) {
                ...
                if (std::regex_match(GetSyscallName(fl->syscallNumber), sbRegex)) {
                    ...
                    info("Skipping, [%d] %s @ 0x%lx | arg0 0x%lx arg1 0x%lx does not match blacklist regex (%s)", pid,
                         GetSyscallName(fl->syscallNumber), fl->pc, fl->arg0, fl->arg1, sbRegexStr.c_str());
                }
                fakeLeaveStalls = 0;
            }
        } else {
        ...
        if (lastPhase == curPhase && scheduledThreads == outQueue.size() && !sleepQueue.empty()) {
            ...
            if (wakeupUsec > 10 * 1000 * 1000)
                warn("Watchdog sleeping for a long time due to long sleep, %ld secs", wakeupUsec / 1000 / 1000);
        ...
        // We could make this self-checking by periodically checking for liveness of the processes we're supposedly
        // running. The bigger problem is that if we get SIGKILL'd, we may not even leave a consistent zsim state
        // behind.
...
void Scheduler::startWatchdogThread() { PIN_SpawnInternalThread(threadTrampoline, this, 64 * 1024, nullptr); }
